...
public final class VaultClientService {
    ...
    private ExecutionResult executeLookupMode(final VaultClientRequest request) {
        LoggingUtils.logStructuredError(
                ...
                EXECUTE_LOOKUP,
                STARTED,
                "Retrieving password for user: " + request.getUser() + DATABASE_LABEL + request.getDatabase(),
                null);

        try (VaultClient vaultClient = new VaultClient()) {
            final String password = vaultClient.fetchOraclePassword(request.getUser(), request.getDatabase());
            ...
            if (password != null && !password.trim().isEmpty()) {
                ...
                LoggingUtils.logStructuredError(
                        ...
                        EXECUTE_LOOKUP,
                        NO_CONFIG,
                        "No vault configuration found for user: " + request.getUser() + DATABASE_LABEL
                                + request.getDatabase(),
                        null);
                return ExecutionResult.failure(1, "[ERROR] No vault configuration found for user: " + request.getUser()
            ...
            LoggingUtils.logStructuredError(
                    ...
                    exception);
            return ExecutionResult.failure(1, "[ERROR] Failed to retrieve password: " + exception.getMessage());
            ...
            return ExecutionResult.failure(1,
                    "[ERROR] Unexpected error during password retrieval: " + exception.getMessage());
        }
    }
        ...
        try (VaultClient vaultClient = new VaultClient()) {
            ...
            if (password != null && !password.trim().isEmpty()) {
                ...
                LoggingUtils.logStructuredError(
                        ...
                        EXECUTE_DIRECT,
                        FAILED,
                        "Failed to retrieve password using direct vault configuration for user: " + request.getUser(),
                        null);
                return ExecutionResult.failure(1,
