...
CREATE ALIAS IF NOT EXISTS TEST_PROC AS '
    String testProc(String inputName, Integer inputValue) {
        // This simulates Oracle procedure behavior
        return "Processed: " + inputName + " with value " + inputValue;
    }
';

-- Create another procedure that returns multiple values
CREATE ALIAS IF NOT EXISTS GET_DATA AS '
    ResultSet getData(Integer id) throws SQLException {
        Connection conn = DriverManager.getConnection("jdbc:h2:mem:testdb");
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM test_data WHERE id = ?");
        stmt.setInt(1, id);
...
CREATE ALIAS IF NOT EXISTS CALCULATE AS '
    Double calculate(Double input1, Double input2, String operation) {
        switch (operation.toLowerCase()) {
            case "add": return input1 + input2;
            case "subtract": return input1 - input2;
            case "multiply": return input1 * input2;
            case "divide": return input2 != 0 ? input1 / input2 : null;
            default: return null;
        }
    }
'; 